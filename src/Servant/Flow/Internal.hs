...
getEnv = fmap (fmap toReferenced) . go []
  where
    ...
    go ns fx = case unfix fx of
        ...
        R1 (Named n b)
            | n `elem` ns -> []
            | otherwise   -> ((n, b) :) . concat . toList $ go (n : ns) <$> unfix b
...
class Flow a where
  ...
  default flowTypeInfo ::
    (Generic a, GFlow (Rep a)) => Proxy a -> FlowTypeInfo
  flowTypeInfo = genericFlowType defaultOptions

genericFlowType
    :: forall a . (Generic a, GFlow (Rep a)) => Options -> Proxy a -> FlowTypeInfo
...
instance (Ord k, FlowObjectKey k, Flow a) => Flow (Map k a) where
    flowTypeInfo _ =
        Fix . L1 . Object $ [IndexerProperty primString $ flowTypeInfo (Proxy @a)]
...
instance (Flow a, Selector s) => GFlowRecordFields (S1 s (K1 R a)) where
    recordFields _ = [(selName (undefined :: S1 s (K1 R a) ()), flowTypeInfo (Proxy @a))]
...
instance Constructor meta => GSimpleSum (M1 C meta U1) where
    simpleSumOptions opts _ = pure . fromString . constructorTagModifier opts $ conName
        (undefined :: C1 meta U1 ())
...
renderFlowTypeF (ExactObject l) =
    inBananaBrackets . T.intercalate ", " $ (\(n, t) -> n <> " : " <> t) <$> l
